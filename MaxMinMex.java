...
public class MaxMinMex {
		...
		public static void main(String[] args) throws IOException  {
			int t = sc.nextInt();
			preCompute = new long[200001];
			preCompute[0] = preCompute[1] = 1;
			for( int i = 2 ; i<= 200000 ;i++) {
				preCompute[i] = (preCompute[i-1]*1L*i)%mod;
			}
		...
		/*
		 * 
		 * maximise the mex right?
		 * 
		 * we for sure know that it will alteast be 1 
		 * 
		 * array R is max - min firs the first element is always 0 
		 * is for sure our mex is altest 1 
		 * 
		 * 
		 * 
		 * now 
		 * 
		 * we know that our array r will always be non decreasing as max-min , the only this can change is 
		 * either there is a new min or there is a new max 
		 * 
		 * 
		 * the only way you will get he maximum mex 
		 * is be having an arrray values like 4 5 6 7 8 9 
		 * 
		 * 
		 * 
		 */
			...
			for( int i = 0 ;i < n;i++) {
				...
				if( curr == max_size) {
					ans = (ans+ (power(2, curr-1 , mod)*preCompute[n-curr])%mod)%mod;
				}
			}
			out.println(ans);
		}
