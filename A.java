		...
		static void run() {
			...
			for( int x : hs) {
				...
				while( temp <= n) {
					...
					dp[(int)temp] = Math.min(dp[(int)temp], count); 
					temp*=x;
					count++;
					if( x == 1) break;
			...
			for( int i = 1; i<= n;i++) {
				TreeSet<Long> ts = allfactors(i);
				for( long x : ts) {
					int f = (int)x;  int s = (int)(i/x);
					
					if( dp[f] != Integer.MAX_VALUE && dp[s] != Integer.MAX_VALUE) {
						dp[i] = Math.min(dp[i], dp[f]+dp[s]);
					}
				}
				
				out.print(dp[i] == Integer.MAX_VALUE? -1 : dp[i]);
				out.print(" ");
